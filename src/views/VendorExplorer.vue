<template>
  <div>
    <header class="mb-6">
      <h1 class="text-2xl font-bold text-white">Vendor Explorer</h1>
      <p class="text-gray-400 mt-1">Browse organizations with known vulnerabilities</p>
    </header>
    ...
    <!-- Empty State -->
    <div v-else-if="!vendors.length" class="bg-dark-600 border border-primary-500/20 rounded-xl p-8 text-center">
      ...
      <h3 class="text-lg font-medium text-white mb-2">No Vendors Found</h3>
      ...
      >
        <div class="flex items-start">
          <div class="flex-shrink-0">
            <div class="w-12 h-12 rounded-full bg-secondary-500/20 flex items-center justify-center text-secondary-400 text-xl font-semibold">
              {{ vendor.name.charAt(0).toUpperCase() }}
            </div>
          </div>
          <div class="ml-4 flex-1">
            <h3 class="text-lg font-medium text-white">{{ vendor.name }}</h3>

            <div class="flex items-center text-sm text-gray-400 mt-1">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
              </svg>
              <span>{{ formatDate(vendor.lastSeen) }}</span>
            </div>
          </div>
        </div>

        <div class="mt-4 grid grid-cols-2 gap-3">
          <div class="bg-danger-500/10 rounded-lg p-3 text-center">
            <div class="text-danger-400 text-lg font-semibold">{{ vendor.cveCount || 0 }}</div>
            <div class="text-xs text-gray-400">CVEs</div>
          </div>

          <div class="bg-secondary-500/10 rounded-lg p-3 text-center">
            <div class="text-secondary-400 text-lg font-semibold">{{ vendor.productCount || 0 }}</div>
    ...
    const fetchVendors = async () => {
      ...
      try {
        ...
        if (searchQuery.value) {
          response = await vendorService.searchVendors(
              searchQuery.value,
              currentPage.value,
              20
          );
        } else {
          response = await vendorService.getAllVendors(
              currentPage.value,
              20,
              sortBy.value,
              sortOrder.value
